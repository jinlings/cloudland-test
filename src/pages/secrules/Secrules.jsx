...
import {
  filterSecruleList,
  fetchSecruleList,
} from "../../redux/actions/SecruleAction";
...
class Secrules extends Component {
  ...
  componentDidMount() {
    ...
    const { secruleList } = this.props.secrule;
    const { handleFetchSecruleList } = this.props;
    if (!secruleList || secruleList.length === 0) {
      handleFetchSecruleList(this.props.match.params.id);
    }
  }
  createSecrules = () => {
  ...
  filter = (event) => {
    this.props.handleFilterSecruleList(event.target.value);
  };
  render() {
    const { filteredList, isLoading } = this.props.secrule;
    return (
      <Card
        title={
...
const mapStateToProps = ({ secrule }) => {
  console.log("mapStateToProps-state", secrule);
  return {
    secrule,
  };
};
...
const mapDispatchToProps = (dispatch) => {
  return {
    handleFetchSecruleList: (secgroupID) =>
      dispatch(fetchSecruleList(secgroupID)),
    handleFilterSecruleList: (keyword) => dispatch(filterSecruleList(keyword)),
  };
};
